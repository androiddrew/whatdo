...
- **Blocking**: Gitea supports issue dependencies (Settings → enable dependencies). Where available, add a "blocked by" edge in the UI or via `POST .../api/v1/repos/androiddrew/laya-server/issues/<child>/dependencies` with `{"index": <blocker>}`. Otherwise fall back to a `Blocked by: #<n>, #<n>` line at the top of the child body. A ticket is unblocked when every blocker is closed.
