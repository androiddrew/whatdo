...
Observability is opt-in and costs nothing when off. The OpenTelemetry libraries live in the `whatdo[otel]` extra (ADR-0002), and **every OTEL import is guarded** inside `configure_observability`, behind the `WHATDO_OTEL__ENABLED` master toggle — so the base install imports and runs without them. A master toggle plus independent per-signal toggles (`traces`/`metrics`/`logs`) and a configurable OTLP endpoint + service name all live in `Settings`. When disabled, the request path records into a no-op `Telemetry` facade that holds no instruments.
